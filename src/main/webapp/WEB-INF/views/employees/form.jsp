 ...
 * 	- 요구사항
 		* 부서를 선택하면 해당 부서에 소속된 직원을 조회해서 관리자 셀렉터박스에 표시한다.
 		1. 관리자 셀렉터 박스 엘리먼트를 조회한다.
 			let el = document.querySelector("select[name=managerId]")
 		2. 관리자 셀렉터 박스 엘리먼트의 내부 HTML 컨텐츠를 전부 삭제한다.
 			el.innerHTML = ""
 		3. 관리자 셀렉터 박스 엘리먼트를 활성화시킨다.
 			el.disabled = false;
 		...
 		6. XHR를 이용해서 서버가 보낸 ajax 응답을 처리한다.
 			...
 			// 배열의 데이터를 처리해서 관리자 셀렉터 박스에 추가한 <option/> 태그를 생성한다.
 			emps.forEach(emp => {
 				oprions += `<option value="\${emp.id}"> \${emp.firstName}</option>`;
 			});
 			// 생성된 <option/>태그를 관리자 셀렉터 박스 엘리먼트에 추가한다.
 			el.innerHTML = options;
 			
 		* Sales(부서아이디 80번) 부서를 선택하면 커미션입력필드를 활성화하고, 그외는 비활성화 한다.
 ...
 */
 	...
 	// 부서를 변경할 때마다 실행되는 이벤트 핸들러 함수다.
	function changeDept () {
		refreshManagerField();
		toggleCommissionField()
	}
	
	// 현재 선택된 부서에 소속된 직원 목록을 조회해서 관리자 셀렉터박스에 표시한다.
	function refreshManagerField() {
		let el = document.querySelector("select[name=managerId]");
		el.innerHTML = "";		// select 박스의 내부 컨텐츠를 전부 지운다.
		el.disable = false;		// select 박스를 활성화 상태로 바꾼다.
