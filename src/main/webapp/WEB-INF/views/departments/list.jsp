...
// 부트스트랩의 모달객체 생성하기
	...
	// hidden.bs.modal은 부트스트랩의 사용자정의 이벤트다.
	// 모달창이 완전히 화면에 보이지 않게 되었을 때(닫혔을 때) 발생하는 이벤트다.
	...
	// 직원목록에서 상세보기버튼을 클릭할 때 실행되는 이벤트 핸들러 함수다.
	// 직원 아이디를 전달받아서 ajax 방식으로 서버에 직원 상세정보 조회 요청을 보낸다.
	...
	function showEmployeeModal(empId) {
		...
		xhr.onreadystatechange = function() {
			...
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelector("#cell-emp-name").textContent = emp.firstName;
				...
				// emp.commissionPct || '' 참이면 전항 거짓이면 후항 대입
				document.querySelector("#cell-emp-comm").textContent = emp.commissionPct || '';
				document.querySelector("#cell-manager-name").textContent = emp.manager ? emp.manager.firstName : '';
				document.querySelector("#cell-dept-name").textContent = emp.department ? emp.department.name : '';
				
				empModal.show();
			}
		}
		// 의도하는 경로는 localhost/emp/detial?id=100
		// 현재 경로는 localhost/dept/list
		// 경로는 현재 url기준으로 옮겨가기 때문에
		// 상대경로를 사용하려면 xhr.open("get", "../emp/detail?id=" + empId)라고 적으면 된다.
